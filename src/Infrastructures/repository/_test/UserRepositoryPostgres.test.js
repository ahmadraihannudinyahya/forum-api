...
describe('UserRepositoryPostgres', () => {
  ...
  describe('userOwnerVerification function', () => {
    it('should throw error when owner not found', async () => {
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});
      await expect(userRepositoryPostgres.userOwnerVerification('user-123')).rejects.toThrowError('USER_REPOSITORY.OWNER_NOT_FOUND');
    });
    it('should not throw error when user owner found', async () => {
      await UsersTableTestHelper.addUser({
        id: 'user-123',
      });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});
      await expect(userRepositoryPostgres.userOwnerVerification('user-123')).resolves.not.toThrowError();
    });
  });
});
